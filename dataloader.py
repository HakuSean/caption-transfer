...
class DataLoader(data.Dataset):
    ...
    def read_files(self):
        ...
        self.feats_attributes = h5py.File(os.path.join(
            self.opt.input_attributes_dir, 'attributes'+self.opt.attribute_opt+'.h5'), 'r')
    ...
    def get_data(self, ix):
        ...
        if self.use_att and self.use_fc:
            return (np.concatenate([np.array(self.feats_fc[index]).astype('float32'),
                    np.array(self.feats_attributes[index]).astype('float32')]),
                    np.array(self.feats_att[index]).astype('float32'), ix)
        elif self.use_att:
            return  (np.array(self.feats_attributes[index]).astype('float32'),
                    np.array(self.feats_att[index]).astype('float32'), ix)            
        elif self.use_fc:
            return (np.concatenate([np.array(self.feats_fc[index]).astype('float32'),
                    np.array(self.feats_attributes[index]).astype('float32')]),
    ...
    def __init__(self, opt, target=False, opt_extra=None, batch_size=None):
        self.opt = opt
        ...
        print('DataLoader loading feature h5 file: ', opt.input_fc_dir,
              opt.input_attributes_dir, opt.input_att_dir, opt.input_label_h5)
        ...
        # # it used to be very complex, because in original info, the coco contains all training\val\test splits, and the only indicator to distinguish them is the index
        ...
        #     elif opt.train_only == 0:  # restval, 30504 for coco
        #         self.split_ix['train'].append(ix)
    ...
    def get_batch(self, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size): # every time, BlobFetcher only get one sample
            ...
            #print('Get one data:', time.time() - start)
            ...
            #print('Arrange label:', time.time() - start)
