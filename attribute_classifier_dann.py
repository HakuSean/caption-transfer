...
class Net(nn.Module):
    def __init__(self, num_class):
        ...
        self.bottleneck = nn.Sequential(nn.Linear(2048, self.neck), nn.ReLU(True), nn.Dropout()) # fc cannot use BN layer
        self.domain_classifier = nn.Sequential(nn.Linear(self.neck, 1024), nn.ReLU(True), nn.Dropout(), nn.Linear(1024, 1))
        self.category_classifier = nn.Sequential(nn.Linear(self.neck, 2048), nn.ReLU(True), nn.Dropout(), nn.Linear(2048, 256))
...
def epoch_map(labels, logits, delta=1e-11):
    precision = [] # final dim: 256(class_num) * 1000(instance number)
    recall = []
...
#output: avearage precision, recall and F1 score
    ...
    for i in range (0, iteration):
        ...
        gt_array.extend(labels)

    gt_array = np.array(gt_array)
    logits_array = np.array(outputs)
    precision, recall = epoch_map(gt_array, logits_array)
    #print('The overall precision is %.4f.' % precision)

    # ap and mAP
    ap = []
    for c in range(len(gt_array[0])): # for each class
        p = precision[c]
        r = recall[c]
    ...
    total_recall = 0
    total_F1 = 0
    for i in range (0, test_Num): # for each test instance
        label = gt_array[i]
        predict = outputs[i]
        #print(label)
        #print(predict)
    ...
    return final_loss, ave_precision, ave_recall, mAP
...
def adjust_learning_rate_dann(lr, optimizer):
...
#parser.add_argument('--weight_files', nargs=2, type=str, default='',
#                    help='input weights if using attend files')
...
parser.add_argument('--proc', type=str, default="mean")
...
weight_files = [os.path.join(os.path.dirname(x), 'weights_'+ args.proc +'.h5') for x in args.feat_files]
if args.use_att:
    weights = [h5py.File(x, 'r') for x in weight_files]
    num_weights = weights[0].values()[0].shape[-1] # 1 x 196
...
source_keys = feats[0].keys() # the cocoid's, are all str(int)'s
shuffle(source_keys)
target_keys = feats[1].keys()

for key in source_keys:
    if args.use_att:
        source_data.append(weights[0][key][:].dot(feats[0][key][:].reshape(num_weights, -1)))
    else:
...
for key in target_keys:
    if args.use_att:
        target_data.append(weights[1][key][:].dot(feats[1][key][:].reshape(num_weights, -1)))
    else:
...
##########################################
    ...
    for epoch in range(epoch_number):  # loop over the dataset multiple times
        ...
        for _, data in enumerate(trainloader, 0): # each time is an iteration
            ...
            lr = learning_rate / (1. + 10 * p)**0.75 # for dann

            adjust_learning_rate_dann(lr, optimizer) # make adjustment every iteration
        ...
        if epoch%10 == 9:
            #train_loss, train_precision, train_recall, train_mAP = result(trainData, trainGT)
            #print('epoch [%d,%d]  Training result: loss= %.6f, precision=%.5f, recall=%.5f, mAP= %.5f' %(epoch + 1, epoch_number, train_loss, train_precision, train_recall, train_mAP))
            test_loss, test_precision, test_recall, test_mAP = result(testData,testGT)
            print('epoch [%d,%d]  validation result: loss= %.6f, precision=%.5f, recall=%.5f, mAP= %.5f'  %(epoch + 1, epoch_number, test_loss, test_precision, test_recall,test_mAP))
            ...
            # print('Best training result(mAP): epoch [%d,%d]  : loss= %.6f, precision=%.5f, recall=%.5f, mAP= %.5f' %(best_train_epoch, epoch_number, best_train_loss, best_train_precision, best_train_recall, best_train_mAP))
            print('Best validation result(mAP): epoch [%d,%d]  : loss= %.6f, precision=%.5f, recall=%.5f, mAP= %.5f' %(best_test_epoch, epoch_number, best_test_loss, best_test_precision, best_test_recall, best_test_mAP))
